    ...
    try{
        ...
        if (typeof transactionData.purchasedItems === "string"){
            try{
                ...
                res.status(400).json({messsage:" Invalid format purchasedItems"})
                return;
            }
        }
        // forcing  status "Pending"
...
export const getTransactions = async (req: Request, res:Response) : Promise<void>  =>{
    try{
        const transaction = await Transaction.find().sort({createdAt: -1}).populate("purchasedItems.productId");
